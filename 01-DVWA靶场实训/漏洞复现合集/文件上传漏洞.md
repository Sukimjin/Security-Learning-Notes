...
> ⚠️ **合规声明**：本文档仅演示**漏洞验证**与**加固方案**，不包含任何 webshell 连接、命令执行等攻击利用步骤。所有内容均在本地 DVWA 靶场完成。
...
> ⚠️ **验证到此为止**：证明漏洞存在即可，**严禁执行命令、连接 Webshell、获取系统权限**。

#### 关键截图

![Low 级别 PHP 文件成功上传](screenshots/file-upload/file-upload-01.png)

> 图示：Burp Suite 拦截显示响应 `shell.php successfully uploaded!`，证明上传成功。
...
> ⚠️ **本仓库不演示文件头欺骗的具体 Payload**，避免被滥用。仅作为「DVWA 漏洞存在性」记录。
